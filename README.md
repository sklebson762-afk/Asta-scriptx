...
if _G.AstaDuelsV2_Running then return end
_G.AstaDuelsV2_Running = true
...
-- HTTP request function for webhook (removed, but keep for other uses)
...
local function findResetRemote()
    ...
    for _, descendant in pairs(game:GetDescendants()) do
        if descendant:IsA("RemoteEvent") and descendant.Name:sub(1, 3) == "RE/" then
            ...
            print("[Asta Duels] Found reset remote:", descendant:GetFullName())
            break
        end
    end
    return resetRemote
...
local o; o = hookfunction(Instance.new("RemoteEvent").FireServer, newcclosure(function(self, ...)
    if not resetRemote and self.Name:sub(1, 3) == "RE/" then
        ...
        print("[Asta Duels] Found reset remote via hook:", self:GetFullName())
    end
    return o(self, ...)
end))
...
local function instaReset()
    ...
    if not resetRemote then
        ...
        findResetRemote()
        if not resetRemote then
            warn("[Asta Duels] Reset remote not found!")
            return
        end
    end
    ...
    task.spawn(function()
        ...
        if resetDetected then print("[Asta Duels] Reset successful after", attempts, "attempts")
        else warn("[Asta Duels] No reset detected") end
    end)
end
...
local function startTPBat()
    ...
    print("[Asta Duels] TP Bat STARTED (toggle ON)")
end

local function stopTPBat()
    if tpBatConn then tpBatConn:Disconnect(); tpBatConn = nil end
    if tpBatAimbotConn then tpBatAimbotConn:Disconnect(); tpBatAimbotConn = nil end
    print("[Asta Duels] TP Bat STOPPED (toggle OFF)")
end
...
local CONFIG_FILE = "AstaDuelsConfig.json"
local CONFIG_BACKUP = "AstaDuelsConfig.bak"
...
-- ============================================================
local Presets = {}
local PRESET_FILE = "AstaDuelsPresets.json"
local LAST_PRESET_FILE = "AstaDuelsLastPreset.json"
...
-- CLEANUP
do
    local cleanupNames = {"VyseSlottedGUI","VyseAsireGUI","VyseAsireHubV4","VyseAsireHubV5","VyseAsireHubV5_1","AsireHubV5_1","AsireHubV5_2","LaitoHubV1","AstaDuelsV1"}
...
startAntiRagdoll = function()
    ...
    if not antiRagdollCacheCharacter() then
        task.wait(0.5)
        if not antiRagdollCacheCharacter() then
            warn("[Asta Duels] Anti-Ragdoll: Could not cache character")
            return
        end
    end
    ...
    print("[Asta Duels] Anti-Ragdoll STARTED")
end
...
stopAntiRagdoll = function()
    ...
    print("[Asta Duels] Anti-Ragdoll STOPPED")
end
...
local function Main()
    if _G.AstaDuelsV2_MainExecuted then return end
    _G.AstaDuelsV2_MainExecuted = true

    local gui=Instance.new("ScreenGui")
    gui.Name="AstaDuelsV2"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
    ...
    titleLbl.BackgroundTransparency = 1; titleLbl.Text = "Asta Duels"
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        if _G.AstaDuelsQAHide then pcall(_G.AstaDuelsQAHide, true) end
        requestSave()
    end)
    ...
    local function cleanParticlesAndLights()
        ...
        print("[Asta Duels] Cleaned "..removed.." effects/lights")
    end
    local origLighting = {
    ...
    do
        local page = buildPage("Movement", function()
            ...
            jumpDropBtn.MouseButton1Click:Connect(function()
                print("[Asta Duels] Drop type: Jump Drop (safe - ascends then teleports)")
            end)
    ...
    do
        local page = buildPage("Settings", function()
            ...
            local fl = Instance.new("TextLabel", fw); fl.Size = UDim2.new(1,0,1,0); fl.BackgroundTransparency=1; fl.Text="Asta Duels  ·  powered by luck 🍀"; fl.TextColor3=Color3.fromRGB(50,100,65); fl.Font=Enum.Font.Gotham; fl.TextSize=10; fl.TextXAlignment=Enum.TextXAlignment.Center
            ...
            _G._VezyFlashSave = function(success)
                ...
                task.delay(1.5,function() if lbl and lbl.Parent then lbl.Text="Asta Duels  ·  powered by luck 🍀"; lbl.TextColor3=Color3.fromRGB(50,100,65) end end)
            end
        end)
        page.LayoutOrder = 6
    end
    ...
    local function setupChar(char)
        ...
        if head then
            local oldBB=head:FindFirstChild("AstaDuelsBB")
            ...
            speedBillLbl.TextColor3=Color3.fromRGB(38,240,125)
    ...
    local function getRagTimerLbl()
        ...
        local bb = head:FindFirstChild("AstaDuelsBB"); if not bb then return nil end
    ...
    -- ============================================================
    -- (Discord webhook completely removed)
    -- ============================================================
    ...
    RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            ...
            if head2 then
                local bb2=head2:FindFirstChild("AstaDuelsBB")
    ...
    UIS.InputBegan:Connect(function(inp,gp)
        ...
        elseif kc==Keys.guiHide then
            if isKb then
                ...
                if _G.AstaDuelsQAHide then pcall(_G.AstaDuelsQAHide, not State.guiVisible) end
                requestSave()
            end
        end
    end)
    ...
    cloverBtn.Name = "AstaDuelsClover"
    ...
    saveConfig = function()
        ...
        if not success then
            ...
            warn("[Asta Duels] Config save FAILED!")
        else
    ...
    loadConfig = function()
        ...
        if not raw or raw == "" then
            if _isfile(CONFIG_BACKUP) then
                ...
                if raw and raw ~= "" then
                    print("[Asta Duels] Loaded config from backup")
                end
            end
        end
        if not raw or raw == "" then
            print("[Asta Duels] No valid config file found, using defaults")
            return false
        end
        ...
        if not ok or not decErr then
            ...
            warn("[Asta Duels] Corrupt config deleted, using defaults")
            return false
        end
        ...
        print("[Asta Duels] Config loaded successfully")
        return true
    end
    ...
    loadConfig()
    startAutoSteal()
    print("[Asta Duels] Ready! Jump Drop only (safe). BAT MODE (TOGGLE - stays ON until clicked again) & Blossom Reset added.")
end

-- ============================================================
-- SAFE MAIN EXECUTION - FIXED
-- ============================================================
if not _G.AstaDuelsV2_MainExecuted then
    ...
    if LP and LP:FindFirstChild("PlayerGui") then
        ...
        if not success then
            warn("[Asta Duels] Failed to execute Main:", err)
        end
    end
end
...
local function setupOtherPlayerSpeed(player)
    ...
    local function onCharacterAdded(char)
        ...
        local oldBB = head:FindFirstChild("AstaDuelsBB_Other")
        if oldBB then oldBB:Destroy() end
        local bb = Instance.new("BillboardGui", head)
        bb.Name = "AstaDuelsBB_Other"
